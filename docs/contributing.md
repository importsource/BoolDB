...
9. **B+Tree node rebalancing on delete** — Currently, deleting from the B+Tree only removes entries without rebalancing (merging underfull nodes). Implement merge/redistribute operations to reclaim space after heavy deletes.
